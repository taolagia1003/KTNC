...
public class SanPhamDAO extends storeAppDAO<SanPham, String> {
	...
	public void insert(SanPham model) {
		...
		XJDBC.update(sql,
//        model.getMaSP(), 
				model.getTenSP(), model.getSoLuong(), model.getGiaBan(), model.getAnh(), model.getGhiChu(),
				model.getMaDM(), model.getSize());
	}

	@Override
	public void update(SanPham model) {
		String sql = "UPDATE SanPham SET TenSP =?, SoLuong=? ,DonGiaBan=? ,Anh=? , GhiChu=? ,MaDM = ?, Size = ? WHERE MaSP = ?";
		XJDBC.update(sql, model.getTenSP(), model.getSoLuong(), model.getGiaBan(), model.getAnh(), model.getGhiChu(),
				model.getMaDM(), model.getSize(), model.getMaSP());
	}
	...
	public boolean checkInsertSP(String maSP, String tenSP, int soLuong, float giaBan, String anh, String ghiChu,
			int maDM, String size) {
		SanPham sp = selectById(maSP);
		if (sp != null) {
			return false;
		} else if (maSP == null && tenSP == null && soLuong <= 0 && giaBan <= 0 && anh == null && ghiChu == null
				&& maDM == 0 && maDM <= 0) {
			return false;
		} else if (maSP == null || tenSP == null || soLuong <= 0 || giaBan <= 0 || anh == null || ghiChu == null
				|| maDM <= 0) {
	...
	public boolean checkUpdateSP(String maSP, String tenSP, int soLuong, float giaBan, String anh, String ghiChu,
			int maDM, String size) {
		SanPham sp = selectById(maSP);
		if (sp != null) {
			return false;
	...
	public boolean checkXoa(String MaSP) {
		SanPham sp = selectById(MaSP);
		if (sp == null) {
			return false;
		} else if (MaSP == null) {
